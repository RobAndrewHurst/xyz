...
Vercel Functions expose the OIDC token on the `x-vercel-oidc-token` request header, not as a runtime environment variable. The root `server.js` handler converts that header into a temporary Google external-account credentials file and sets `GOOGLE_APPLICATION_CREDENTIALS` before the app imports Varlock. Varlock then uses the standard ADC path to resolve `gsm()` values.
