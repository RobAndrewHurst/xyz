var Mt=Object.defineProperty;var Vt=(e,t)=>{for(var i in t)Mt(e,i,{get:t[i],enumerable:!0})};async function N(e){if(!(!e.dynamic&&typeof e.create=="function")){if(typeof e.create=="function"){e.create();return}if(typeof e.target=="string"&&(e.target=document.getElementById(e.target)),!(e.target instanceof HTMLElement)){console.warn("Dataviews require a HTMLHtmlElement target");return}return Rt(e)instanceof Error?e.err:(typeof e.label=="string"&&(e.chkbox=mapp.ui.elements.chkbox({checked:!!e.display,data_id:e.key,disabled:e.disabled,label:e.label,onchange:t=>{e.display=t,e.display?e.show():e.hide()}})),e.update??=Ut,e.show??=Pt,e.hide??=Ht,mapp.ui.utils.dataview.Toolbar(e),mapp.ui.utils.dataview.mapChange(e),e)}}async function Pt(){this.display=!0,this.create===void 0?(this.create=()=>{mapp.ui.utils[this.dataview].create(this)},this.create(),typeof this.update=="function"&&this.update()):this.dynamic?(typeof this.create=="function"&&await this.create(),typeof this.update=="function"&&this.update()):this.reload&&typeof this.update=="function"&&this.update(),this.btnRow?.style.setProperty("display","flex"),this.target.style.display="block"}function Ht(){this.display=!1,this.target.style.display="none",this.btnRow?.style.setProperty("display","none")}function Rt(e){if(e.key??=e.query||e.title||e.label,e.dataview||(e.chart&&(e.dataview="chartjs"),typeof e.columns<"u"&&(console.warn("Table dataviews should be configured inside a tables object"),e.table={columns:e.columns},e.dataview="tabulator"),e.table&&(e.dataview="tabulator")),!Object.hasOwn(mapp.ui.utils,e.dataview))return e.err=new Error(`mapp.ui.utils.${e.dataview} doesnt exist`),console.error(e.err),e.update=()=>console.warn(`Unable to update ${e.key} dataview.`),e.err;if(typeof mapp.ui.utils[e.dataview].create!="function")return e.err=new Error(`mapp.ui.utils.${e.dataview}.create() method doesn't exist`),console.error(e.err),e.err}async function Ut(){if(this.create===void 0&&(this.create=function(){mapp.ui.utils[this.dataview].create(this)},this.create()),!this.query)return;let e=mapp.utils.queryParams(this),t=mapp.utils.paramString(e);this.host??=this.layer?.mapview?.host;let i=await mapp.utils.xhr(`${this.host||mapp.host}/api/query?${t}`);if(!(i instanceof Error)){if(typeof this.responseFunction=="function"){this.responseFunction(i);return}typeof this.setData=="function"&&this.setData(i)}}function I(e){typeof e=="string"&&(e={text:e}),e.title??=mapp.dictionary.information,e.icon??="info",e.data_id??="alert",e.class??="alert-confirm box-shadow no-select";let t=e.icon===""?"":mapp.utils.html`<span class="material-symbols-outlined notranslate">${e.icon}</span>`;return e.header=mapp.utils.html`${t}${e.title}`,e.innerContent??=mapp.utils.html`<p>${e.text}</p>`,e.content=mapp.utils.html`
    ${e.innerContent}
    <div class="buttons">
      <button 
  ...
  <div 
    ...
    <span>${e.label}`}function F(e){typeof e=="string"&&(e={text:e}),e.title??=mapp.dictionary.confirm,e.data_id??="confirm",e.icon??="warning";let t=e.icon===""?"":mapp.utils.html`<span class="material-symbols-outlined notranslate">${e.icon}</span>`;e.header=mapp.utils.html`${t}${e.title}`,delete e.minimizeBtn,delete e.closeBtn;let i=mapp.utils.html.node`<button 
      class="action outlined bold">
      ...
      <div class="buttons">
        ${i}  
        ${o}`,e.class??="alert-confirm box-shadow no-select",mapp.ui.elements.dialog(e),new Promise(l=>{i.addEventListener("click",()=>{l(!0),e.close()}),o.addEventListener("click",()=>{l(!1),e.close()})})}var Ye={draw:Jt,modify:Wt};function Wt(e){e?.preventDefault?.();let t=[];t.push(mapp.utils.html`
    <li
    ...
    <li
      ...
      ${mapp.dictionary.cancel}`),this.popup({content:mapp.utils.html.node`<ul>${t}`,coords:this.interaction.vertices[this.interaction.vertices.length-1]})}function Jt(e){if(this.interaction.vertices.length===0)return;let t=[];t.push(mapp.utils.html`
  <li
    onclick=${()=>this.interaction.finish(this.interaction.getFeature())}>
      ${mapp.dictionary.save}`),t.push(mapp.utils.html`
    <li
      onclick=${()=>this.interaction.finish()}>
      ${mapp.dictionary.cancel}`),setTimeout(()=>this.popup({autoPan:!0,content:mapp.utils.html.node`<ul>${t}`,coords:this.interaction.vertices[this.interaction.vertices.length-1]}),100)}function A(e){if(e.target)return e.target.classList.add("mapp-control"),e.tabs=mapp.utils.html.node`<nav class="tabs hover">`,e.target.append(e.tabs),e.panels=mapp.utils.html.node`<div class="panels">`,e.target.append(e.panels),e.add=Gt,e}function Gt(e){e.parent=this,e.minWidth??=0,e.icon??=e.key,e.classList??="notranslate material-symbols-outlined",e.onClick??=Zt,e.title??=mapp.dictionary[e.key]||e.key,e.panel??=mapp.utils.html.node`<div>`,this.tabs.children.length||(e.classList+=" active"),e.tab=mapp.utils.html.node`<div
    data-id=${e.key}
    title=${e.title}
    onclick=${t=>e.onClick(t,e)}
    class=${e.classList}>
    ${e.icon}`,this.tabs.append(e.tab),this.panels.append(e.panel)}function Zt(e,t){for(let i of Array.from(t.parent.tabs.children))i.classList.remove("active");t.tab.classList.add("active");for(let i of Array.from(t.parent.panels.children))i.classList.remove("active");t.panel.classList.add("active"),t.focus&&(t.focus=t.focus instanceof HTMLElement?t.focus:document.querySelector(`[name=${t.focus}]`),t.focus.focus())}function B(e){if(!e.new&&typeof e.show=="function"){e.show();return}if(e.show=Xe,!e.target)e.modal=!0;else if(e.target instanceof HTMLElement)e.show=Xe;else return;document.querySelector("dialog.modal")?.close(),e.minimizeBtn&&=mapp.utils.html`<button
    data-id="minimize"
    class="minimize-btn notranslate material-symbols-outlined"
    onclick=${a=>{a.target.closest("dialog").classList.toggle("minimized")}}>`;function t(a){typeof e.onClose=="function"&&e.onClose(a),e.node.remove()}e.close=t,e.closeBtn&&=mapp.utils.html`<button
    data-id=close
    ...
    class=${e.class}>
    ${e.headerHtml}
    <div class="content">${e.content}`,e.modal?(document.body.append(e.node),e.node.showModal()):(e.target.appendChild(e.node),e.node.show(),e.right&&(e.left=e.target.offsetWidth-e.node.offsetWidth-Number.parseInt(e.right)),e.node.style.top=`${e.top||0}`,e.node.style.left=`${e.left||0}`,e.node.addEventListener("mousedown",i),e.node.addEventListener("touchstart",i),new ResizeObserver(o).observe(e.target));function i(a){if(e.headerDrag&&!a.target.closest("header")||a.which===3)return;e.node.style.cursor="grabbing";let s={mousedown:{end:"mouseup",move:"mousemove"},touchstart:{end:"touchend",move:"touchmove"}},{move:d,end:c}=s[a.type];e.target.addEventListener(d,n),window.addEventListener(c,l)}function o(){let{offsetWidth:a,offsetHeight:s}=e.target,{offsetWidth:d,offsetHeight:c}=e.node,u=a-d<0?0:a-d,b=s-c<0?0:s-c;e.node.style.left=`${Math.min(Math.max(e.node.offsetLeft,0),u)}px`,e.node.style.top=`${Math.min(Math.max(e.node.offsetTop,0),b)}px`}function l(){delete e.x,delete e.y,e.node.style.cursor=e.headerDrag?"auto":"grab",e.target.removeEventListener("mousemove",n),e.target.removeEventListener("touchmove",n),e.target.removeEventListener("mouseup",l),e.target.removeEventListener("touchend",l)}function n(a){e.x??=a.x;let s=a.x-e.x;e.x=a.x,e.y??=a.y;let d=a.y-e.y;if(e.y=a.y,e.contained&&!e.modal){Yt(e,s,d);return}if(e.containedCentre&&!e.modal){Xt(e,s,d);return}e.node.style.left=`${e.node.offsetLeft+s}px`,e.node.style.top=`${e.node.offsetTop+d}px`}return e.dialog=e,e}function Yt(e,t,i){e.node.offsetLeft+t<0?e.node.style.left=0:t<0?e.node.style.left=`${e.node.offsetLeft+t}px`:e.target.offsetWidth-e.node.offsetWidth-e.node.offsetLeft>0&&(e.node.style.left=`${e.node.offsetLeft+t}px`),e.node.offsetTop+i<0?e.node.style.top=0:i<0?e.node.style.top=`${e.node.offsetTop+i}px`:e.target.offsetHeight-e.node.offsetHeight-e.node.offsetTop>0&&(e.node.style.top=`${e.node.offsetTop+i}px`)}function Xt(e,t,i){e.node.offsetLeft+Number.parseInt(e.node.offsetWidth/2)+t<0||(t<0?e.node.style.left=`${e.node.offsetLeft+t}px`:e.target.offsetWidth-Number.parseInt(e.node.offsetWidth/2)-e.node.offsetLeft>0&&(e.node.style.left=`${e.node.offsetLeft+t}px`),!(e.node.offsetTop+Number.parseInt(e.node.offsetHeight/2)+i<0)&&(i<0?e.node.style.top=`${e.node.offsetTop+i}px`:e.target.offsetHeight-Number.parseInt(e.node.offsetHeight/2)-e.node.offsetTop>0&&(e.node.style.top=`${e.node.offsetTop+i}px`)))}function Xe(){this.node&&this.target instanceof HTMLElement&&this.target.append(this.node)}function Qe(e){if(e.data_id??="drawer",e.dialog)return M(e);if(e.drawer===!1)return e.drawer=mapp.utils.html.node`<div
  ...
  <div
    ...
    <button
      ...
      ${e.draw.point.label}`,e.draw.point.btn}function ii(e){let t={content:mapp.utils.html.node`<li>
    <ul>${mapp.dictionary.draw_dialog_begin_drawing}</ul>
    ...
    <button
      ...
      ${e.draw.line.label}`,e.draw.line.btn}function oi(e){let t={content:mapp.utils.html.node`<li>
    <ul>${mapp.dictionary.draw_dialog_begin_drawing}</ul>
    ...
    <button
      ...
      ${e.draw.polygon.label}`,e.draw.polygon.btn}function li(e){let t={content:mapp.utils.html.node`<li>
    <ul>${mapp.dictionary.draw_dialog_begin_drawing}</ul>
  ...
  <button
    ...
    ${e.draw.rectangle.label}`,e.draw.rectangle.btn}function ni(e){let t={content:mapp.utils.html.node`<li>
    <ul>${mapp.dictionary.draw_dialog_begin_drawing}</ul>
  ...
  <button
    ...
    ${e.draw.circle_2pt.label}`,e.draw.circle_2pt.btn}function ai(e){let i={helpDialog:{content:mapp.utils.html.node`<li>
    <ul>${mapp.dictionary.draw_dialog_begin_drawing}</ul>
    ...
    <h3>${mapp.dictionary.circle_config}</h3>`,content:o,popout:i.popout,dialog:i.dialog}),i.drawer)}function Ke(e,t){let i=e.unitsConfig[t];e.units=i.option,e.radius??=i.min,e.radius=e.radius>i.max?i.max:e.radius,mapp.utils.render(e.rangeSlider,mapp.ui.elements.slider({callback:o=>{e.radius=Number.parseFloat(o)},max:i.max,min:i.min,val:e.radius}))}function si(e){return e.draw.locator={label:mapp.dictionary.draw_position,layer:e,type:"Point",...e.draw.locator},e.draw.locator.btn=mapp.utils.html.node`
    <button
      class="action wide"
      onclick=${t=>{mapp.utils.getCurrentPosition(async i=>{let o={layer:e,new:!0,table:e.tableCurrent()},l=ol.proj.transform([Number.parseFloat(i.coords.longitude),Number.parseFloat(i.coords.latitude)],"EPSG:4326",`EPSG:${e.srid}`);o.id=await mapp.utils.xhr({body:JSON.stringify({[e.geom]:{coordinates:l,type:"Point"}}),method:"POST",url:`${e.mapview.host}/api/query?`+mapp.utils.paramString({layer:e.key,locale:e.mapview.locale.key,table:o.table,template:"location_new"})}),mapp.location.get(o)})}}>
      <span class="notranslate material-symbols-outlined">my_location</span>
      ${e.draw.locator.label}`,e.draw.locator.btn}function V(e){e.selectedTitles=new Set,e.selectedOptions=new Set,e.placeholder??=e.span||"",e.entries=e.entries?.filter?.(o=>o.option!==""),ci(e),ri(e),e.onChange??=it,e.options=di(e);let t=e.selectedTitles.size?Array.from(e.selectedTitles).join(", "):e.placeholder,i=e.keepPlaceholder?e.placeholder:t;return e.placeHolderOption=mapp.utils.html.node`
    ...
    ${e.pills?.container}
    ${e.search||e.select}`,e.node}function tt(e){e.target.selectedIndex=0}function it(e,t){let i=e.target.selectedIndex;e.target.selectedIndex=0;let o=t.entries[i-1];if(o.selected=!o.selected,t.multi){if(t.options[i].classList.toggle("selected")?(t.selectedTitles.add(o.title),t.selectedOptions.add(o.option),t.pills?.add(o.title)):(t.selectedTitles.delete(o.title),t.selectedOptions.delete(o.option),t.pills?.remove(o.title)),!t.pills&&!t.keepPlaceholder){let n=t.selectedTitles?.size&&Array.from(t.selectedTitles).join(", ");t.placeHolderOption.textContent=n||t.placeholder}t.callback?.(e,[...t.selectedOptions],o);return}t.options.forEach(l=>{l.classList.remove("selected"),l.style.backgroundColor="var(--color-base-secondary)"}),t.options[i].classList.add("selected"),t.options[i].style.removeProperty("background-color"),t.keepPlaceholder||(t.placeHolderOption.textContent=o.title),t.callback?.(e,o)}function di(e){return e.entries.map(i=>(i.li=mapp.utils.html.node`<option
        value=${i.option}>
        ${i.title||i.label||i.field}`,i.selected&&(i.li.classList.add("selected"),e.selectedTitles.add(i.title),e.selectedOptions.add(i.option),e.pills?.add(i.title)),i.li))}function ci(e){e.pills&&(e.pills=mapp.ui.elements.pills({addCallback:(t,i)=>{e.callback?.(null,[...i])},pills:[...e.selectedTitles],removeCallback:(t,i)=>{let o=e.entries.findIndex(n=>n.option===t),l=e.entries.findIndex(n=>n.option===t)+1;e.entries.find(n=>n.option===t).selected=!1,e.options[l].classList.remove("selected"),e.selectedTitles.delete(o.title),e.selectedOptions.delete(o.option),e.callback?.(null,[...i])}}))}function ri(e){if(!e.search)return;let t=`${e.field}-search-options`,i=e.placeholder||"Enter search term...",o=mapp.utils.html.node`<input
    placeholder=${i}
    type="search" list=${t}
    onInput=${n=>ui(n,e)}
    onfocus="this.placeholder=''"
    onblur=${n=>n.target.placeholder=i}>`;e.searchOptions=[];for(let n of e.entries){let a=mapp.utils.html.node`
      <option value=${n.option} data-title=${n.title}>${n.title}`;e.searchOptions.push(a)}let l=mapp.utils.html.node`<datalist id=${t}>${e.searchOptions}`;return e.search=mapp.utils.html.node`${o}${l}`,e.search}function ui(e,t){t.entries.forEach(i=>{i.title===e.target.value&&(i.selected=!i.selected,i.selected?(t.selectedTitles.add(i.title),t.selectedOptions.add(i.option),t.pills?.add(i.title)):(t.selectedTitles.delete(i.title),t.selectedOptions.delete(i.option),t.pills?.remove(i.title)),e.target.value="",e.target.dispatchEvent(new Event("blur")),t.pills||t.callback?.(e,[...t.selectedOptions]))})}var P;function H(e){P?.node.remove(),e&&(mapp.user?.hideHelp||(P={closeBtn:!0,contained:!0,css_style:"padding: 0.5em;",headerDrag:!0,height:"auto",left:"6em",minimizeBtn:!0,target:document.getElementById("Map"),top:"3em",...e},mapp.ui.elements.dialog(P)))}async function R(e){let t=await mapp.utils.esmImport("vanilla-jsoneditor@3.3.1");return e.content??={json:{}},typeof e.data=="object"&&(e.content.json=e.data),t.createJSONEditor({props:{content:e.content,...e.props},target:e.target})}var U={hover:mi,hovers:fi,icon_scaling:$i,label:hi,labels:gi,opacitySlider:bi,panel:pi,theme:vi,themes:wi},ot=U;function pi(e){if(!e.style)return;e.style.elements??=Object.keys(e.style);let t=[];for(let i of e.style.elements){if(!Object.hasOwn(e.style,i)||!Object.hasOwn(U,i))continue;let o=U[i](e);o&&t.push(o)}if(t.length)return e.style.view=mapp.utils.html.node`<div>${t}`,e.style.view}function mi(e){if(e.style.hover&&!e.style.hover.hidden)return mapp.ui.elements.chkbox({checked:!!e.style.hover.display,data_id:`hoverCheckbox-${e.key}`,label:e.style.hovers&&mapp.dictionary.layer_style_display_hover||e.style.hover.title||mapp.dictionary.layer_style_display_hover,onchange:t=>{e.style.hover.display=t}})}function fi(e){if(!e.style.hover||e.style.hover.hidden||!e.style.hovers||Object.keys(e.style.hovers).length<2)return;let t=Object.keys(e.style.hovers).filter(i=>!e.style.hovers[i].hidden).map(i=>({option:i,title:e.style.hovers[i].title||i}));return mapp.ui.elements.dropdown({callback:(i,o)=>{let l=e.style.hover.display;e.style.hover=e.style.hovers[o.option],e.style.hover.method??=mapp.layer.featureHover,e.style.hover.display=l},data_id:`hoversDropdown-${e.key}`,entries:t,placeholder:e.style.hover.title})}function hi(e){if(!e.style.label||e.style.label.hidden)return;if(e.style.labelCheckbox)return e.style.labelCheckbox;let t=e.style.labels&&mapp.dictionary.layer_style_display_labels;return t??=e.style.label.title||mapp.dictionary.layer_style_display_labels,e.style.labelCheckbox=mapp.ui.elements.chkbox({checked:!!e.style.label.display,data_id:`labelCheckbox-${e.key}`,label:t,onchange:i=>{e.style.label.display=i,e.reload()}}),e.changeEndCallbacks.push(lt),e.style.labelCheckbox}function lt(e){if(e.style.label.minZoom||e.style.label.maxZoom){let t=e.mapview.Map.getView().getZoom();t<=e.style.label.minZoom||t>=e.style.label.maxZoom?e.style.labelCheckbox?.classList.add("disabled"):e.style.labelCheckbox?.classList.remove("disabled")}}function gi(e){if(!e.style.label||e.style.label.hidden||!e.style.labels||Object.keys(e.style.labels).length<2)return;let t=Object.keys(e.style.labels).filter(o=>!e.style.labels[o].hidden).map(o=>({option:o,title:e.style.labels[o].title||o}));return mapp.utils.html`
  <div>${mapp.dictionary.layer_style_select_label}</div>
  ${mapp.ui.elements.dropdown({callback:i,data_id:"labelsDropdown",entries:t,placeholder:e.style.label.title})}`;function i(o,l){let n=e.style.label.display;e.style.label=e.style.labels[l.option],e.style.label.display=n,e.reload(),lt(e)}}function bi(e){return mapp.ui.elements.slider({callback:t=>{e.L.setOpacity(Number.parseFloat(t/100))},data_id:"opacitySlider",label:`${mapp.dictionary.layer_style_opacity}`,max:100,min:0,val:Number.parseInt(e.L.getOpacity()*100)})}function vi(e){if(!e.style.theme)return;e.style.labels&&Object.hasOwn(e.style.labels,e.style.theme?.setLabel)&&(e.style.label=e.style.labels[e.style.theme.setLabel]),e.style.hovers&&Object.hasOwn(e.style.hovers,e.style.theme?.setHover)&&(e.style.hover=e.style.hovers[e.style.theme.setHover]);let t=[];return e.style.theme?.meta&&(e.style.theme.meta_node=mapp.utils.html.node`<p>${e.style.theme.meta}`,t.push(e.style.theme.meta_node)),Object.hasOwn(mapp.ui.layers.legends,e.style.theme?.type)&&(mapp.ui.layers.legends[e.style.theme.type](e),e.style.legend&&t.push(e.style.legend)),mapp.utils.html.node`<div data-id="layerTheme">${t}`}function wi(e){if(!e.style.themes||Object.keys(e.style.themes).length<2)return;let t=Object.keys(e.style.themes).map(l=>({option:l,title:e.style.themes[l].title||l}));function i(l,n){e.style.theme=e.style.themes[n.option],e.style.theme.setLabel&&e.style.labels&&(e.style.label=e.style.labels[e.style.theme.setLabel]),e.style.theme.setHover&&e.style.hovers&&(e.style.hover=e.style.hovers[e.style.theme.setHover],e.style.hover.method??=mapp.layer.featureHover);let a=mapp.ui.layers.panels.style(e);e.style.panel&&e.style.panel.replaceChildren(...a.children),e.view?.querySelector("[data-id=style-drawer]").replaceChildren(...a.children),e.reload()}return mapp.utils.html`<div>
    ${mapp.dictionary.layer_style_select_theme}
    ${mapp.ui.elements.dropdown({callback:i,data_id:"themesDropdown",entries:t,placeholder:e.style.theme.title})}`}function $i(e){if(!e.style.icon_scaling||e.style.icon_scaling.hidden)return;let t=[];if(e.style.icon_scaling.fields){let i=Object.keys(e.style.icon_scaling.fields).map(l=>({option:e.style.icon_scaling.fields[l].field,title:e.style.icon_scaling.fields[l].title||l,key:l}));i.push({title:mapp.dictionary.icon_scaling_no_scaling}),e.style.icon_scaling.placeholder??=mapp.dictionary.icon_scaling_select_one,e.style.icon_scaling.title??=mapp.dictionary.icon_scaling_title;let o=Object.keys(e.style.icon_scaling.fields).find(l=>e.style.icon_scaling.fields[l].field===e.style.icon_scaling.field)||Object.keys(e.style.icon_scaling.fields)[0];t.push(mapp.utils.html`
        <h3>${e.style.icon_scaling.title}`,mapp.ui.elements.dropdown({data_id:"iconScalingFieldsDropdown",entries:i,callback:(l,n)=>{n.option?Object.assign(e.style.icon_scaling,{...e.style.icon_scaling.fields[n.key]}):delete e.style.icon_scaling.field,e.reload(),e.L.changed()},placeholder:e.style.icon_scaling.fields[o].title||o||e.style.icon_scaling.placeholder}))}else e.style.icon_scaling.field&&t.push(mapp.ui.elements.chkbox({checked:!!e.style.icon_scaling.field,data_id:`iconScalingFieldCheckbox-${e.key}`,label:mapp.dictionary.icon_scaling_field,onchange:i=>{i?(e.style.icon_scaling.field=e.style.icon_scaling._field,delete e.style.icon_scaling._field):(e.style.icon_scaling._field=e.style.icon_scaling.field,delete e.style.icon_scaling.field),e.L.changed()}}));return e.style.icon_scaling.icon&&(e.style.default.scale??=1,e.style.icon_scaling.maxScale??=e.style.default.scale*3,e.style.icon_scaling.minScale??=.1,e.style.icon_scaling.icon_scaling_label??=mapp.dictionary.icon_scaling_label,t.push(mapp.ui.elements.slider({callback:i=>{e.style.default.scale=Number.parseFloat(i),clearTimeout(e.style.timeout),e.style.timeout=setTimeout(()=>e.L.changed(),400)},data_id:"iconScalingSlider",label:e.style.icon_scaling.icon_scaling_label,max:e.style.icon_scaling.maxScale,min:e.style.icon_scaling.minScale,step:e.style.default.scale/10,val:e.style.default.scale}))),e.style.icon_scaling.clusterScale&&e.style.cluster?.clusterScale&&(e.style.cluster.clusterScale??=1,t.push(mapp.ui.elements.slider({callback:i=>{e.style.cluster.clusterScale=Number.parseFloat(i),clearTimeout(e.style.timeout),e.style.timeout=setTimeout(()=>e.L.changed(),400)},data_id:"iconScalingClusterSlider",label:mapp.dictionary.icon_scaling_cluster,max:e.style.cluster.clusterScale*3,min:0,step:e.style.cluster.clusterScale/10,val:e.style.cluster.clusterScale}))),e.style.icon_scaling.zoomInScale&&(e.style.zoomInScale??=1,t.push(mapp.ui.elements.slider({callback:i=>{e.style.zoomInScale=Number.parseFloat(i),clearTimeout(e.style.timeout),e.style.timeout=setTimeout(()=>e.L.changed(),400)},data_id:"iconScalingZoomInSlider",label:mapp.dictionary.icon_scaling_zoom_in,max:e.style.zoomInScale*3,min:0,step:e.style.zoomInScale/10,val:e.style.zoomInScale}))),e.style.icon_scaling.zoomOutScale&&(e.style.zoomOutScale??=1,t.push(mapp.ui.elements.slider({callback:i=>{e.style.zoomOutScale=Number.parseFloat(i),clearTimeout(e.style.timeout),e.style.timeout=setTimeout(()=>e.L.changed(),400)},data_id:"iconScalingZoomOutSlider",label:mapp.dictionary.icon_scaling_zoom_out,max:e.style.zoomOutScale*3,min:0,step:e.style.zoomOutScale/10,val:e.style.zoomOutScale}))),mapp.utils.html.node`<div>${t}`}var nt=new XMLSerializer;function W(e){let t=[];return Array.isArray(e.icon)?t.push(_i(e)):(e.icon||e.svg||e.type)&&t.push(xi(e)),e.fillColor&&t.push(yi(e)),e.strokeColor&&!e.fillColor&&t.push(ki(e)),mapp.utils.html.node`<div class="legend-icon">${t}`}function _i(e){let t=document.createElement("canvas");t.width=e.width,t.height=e.height;let i=e.icon.length;function o(){if(--i)return;let n=ol.render.toContext(t.getContext("2d"),{pixelRatio:1,size:[e.width,e.height]});e.icon.forEach(a=>{n.setStyle(a.legendStyle),n.drawGeometry(new ol.geom.Point([t.width*.5,t.height*.5]))})}let l=e.icon[0].legendScale||1;for(let n of e.icon){let a=n.url||n.svg||at(n);if(!a){console.warn("legendIcon: icon has no url, the svg or template may be missing/invalid: ",n);continue}let s=new ol.style.Icon({anchor:n.legendAnchor||[.5,.5],crossOrigin:"anonymous",scale:l*(n.scale||1),src:a});n.legendStyle=new ol.style.Style({image:s});let d=s.getImage();s.getImageState()===2?o():(d.addEventListener("load",o),s.load())}return t}function xi(e){let t=e.icon?.svg||e.svg||e.icon?.url||e.url||at(e.icon||e);t||console.warn("legendIcon: icon has no url, the svg or template may be missing/invalid: ",e.icon||e);let i=`
    background-position: center;
    background-repeat: no-repeat;
    background-size: contain;
    width: ${e.width+"px"||"100%"};
    height: ${e.height+"px"||"100%"};
    background-image: url(${t})`;return mapp.utils.html.node`<div style=${i}>`}function at(e){if(e.type??="dot",!!Object.hasOwn(mapp.utils.svgSymbols,e.type))return mapp.utils.svgSymbols[e.type]}function ki(e){let t=`M 0,${e.height/2} L ${e.width/2},${e.height/2} ${e.width/2},${e.height/2} ${e.width},${e.height/2}`;e.strokeWidth??=1,e.lineDash??="";let i=mapp.utils.svg.node`
  <svg
    height=${e.height}
    width=${e.width}>
    <path
      ...
      stroke-dasharray=${e.lineDash} />`,o=`data:image/svg+xml,${encodeURIComponent(nt.serializeToString(i))}`,l=`
    background-position: center;
    background-repeat: no-repeat;
    background-size: contain;
    width: ${e.width}px;
    height: ${e.height}px;
    background-image: url(${o});`;return mapp.utils.html`<div style=${l}>`}function yi(e){e.fillOpacity??=1,e.strokeWidth??=1,e.lineDash??="";let t=mapp.utils.svg.node`
  <svg
    height=${e.height}
    width=${e.width}>
    <rect
    ...
    <a 
      class="switch"
      href=${e.register}>${e.login_registration_link}</a>`;function s(){n.disabled=!(i.validity.valid&&l.validity.valid),o.textContent=n.disabled?e.login_invalid:""}}function Z(e){e.placeholder??="",e.data_id??="numeric-input",e.numericChecks??=Li;let t=mapp.utils.formatNumericValue(e),o=`text-align: center; width: ${e.dynamicWidth?t.length+1.3+"ch":"100%"}`;return mapp.utils.html.node`<input
    data-id=${e.data_id}
    type="text"
    style=${o}
    ...
    oninput=${n=>dt(n,e)}>`}function dt(e,t){if(t.stringValue=e.target.value,t.numericChecks(e.target.value,t)?(t.newValue=t.onRangeInput?t.stringValue:mapp.utils.unformatStringValue(t),delete t.invalid,e.target.classList.remove("invalid"),t.sliderElement&&(t.sliderElement.style.setProperty(`--${e.target.dataset.id}`,t.newValue),t.sliderElement.querySelector(`[name=${t.rangeInput}]`).value=t.newValue)):(t.invalid=!0,e.target.classList.add("invalid")),t.invalid)return t.callback();t.callback(t.newValue),t.dynamicWidth&&(e.target.style.width=e.target.value.length+1.3+"ch"),t.onRangeInput=!1}function Li(e,t){return t.onRangeInput&&e===null||Number.isNaN(Number(e))||t.min&&e<t.min?!1:t.max?e<=t.max:!0}function Y(e={}){return e.container=mapp.utils.html.node`<div class="pill-container">`,e.pills=Array.isArray(e.pills)?new Set(e.pills):new Set,e.add=Ci,e.remove=Ei,e.pills.forEach(t=>e.add(t)),e.target instanceof HTMLElement&&e.target.append(e.container),e}function Ci(e){let t=this,i=mapp.utils.html.node`<div
    class="pill"
    style=${t.css_pill}
    data-value=${e}
    ...
    <span>${e.label}</span>`;return e.dialog&&(e.header??=e.label,t.onclick=i=>{i.preventDefault(),Si(e)}),t}function Si(e){let t=mapp.utils.html`<iframe src=${e.href}>`;e.dialog=mapp.ui.elements.dialog({header:mapp.utils.html`<h2>&nbsp;${e.header}`,css_style:"width: 100%; height: 100%",containedCentre:!0,closeBtn:!0,content:t})}function ee(e={}){if(typeof e.searchFunction!="function"){console.warn("A searchFunction must be provided for the construction of a searchbox component.");return}return e.target instanceof HTMLElement||(e.target=mapp.utils.html.node`<div>`),e.name??="searchbox-input",e.input=mapp.utils.html.node`
  <input
    name=${e.name}
    type="search"
    placeholder=${e.placeholder}
    ...
    <div
      ...
      <div class="actions">
      ${o}
      </div>`;document.body.append(n);function a(s,d){n.classList.add("ease-out"),setTimeout(function(){n.remove()},1200),typeof d?.callback=="function"&&d.callback(s),t(s?.target.value)}})}function le(e={}){if(e.data_id??="ui-elements-tooltip",!e.content){console.warn(`mapp.ui.elements.tooltip: content for ${e.data_id} not provided.`);return}return e.node=mapp.utils.html.node`<span 
    data-id=${e.data_id}
    title=${e.content}
  ...
  <div>
    ...
    <ul>${o}`)}var ct={alert:I,btnPanel:Ze,card:z,chkbox:D,confirm:F,contextMenu:Ye,control:A,dialog:B,drawer:Qe,drawerDialog:M,drawing:et,dropdown:V,helpDialog:H,jsoneditor:R,layerStyle:ot,legendIcon:W,locales:J,loginForm:G,numericInput:Z,pills:Y,radio:X,reportLink:K,registerForm:Q,searchbox:ee,slider:te,slider_ab:ie,toast:oe,tooltip:le,userLocale:ne};var rt=e=>{e={...e,...mapp.ui.elements.searchbox({name:"gazetteer-search-input",placeholder:e.placeholder,searchFunction:t,target:e.target})};function t(i){if(e.list.innerHTML="",!i.target.value.length)return;let o=i.target.value.split(",").map(parseFloat);if(o.length===2&&o.every(l=>typeof l=="number"&&!Number.isNaN(Number(l))&&Number.isFinite(Number(l)))){let[l,n]=o;if(l>=-90&&l<=90&&n>=-180&&n<=180){e.list.appendChild(mapp.utils.html.node`
        <li onclick=${a=>{mapp.utils.gazetteer.getLocation({label:`Latitude:${o[0]}, Longitude:${o[1]}`,lat:o[0],lng:o[1],source:"Coordinates"},e)}}><span>Latitude:${o[0]}, Longitude:${o[1]}</span>`);return}else e.list.appendChild(mapp.utils.html.node`
          <li style="color: red;">
            <span>${mapp.dictionary.invalid_lat_long_range}</span>`)}e.provider&&(Object.hasOwn(mapp.utils.gazetteer,e.provider)?mapp.utils.gazetteer[e.provider](i.target.value,e):console.warn("Requested gazetteer service not available")),mapp.utils.gazetteer.datasets(i.target.value,e)}};var f={applyFilter:Oi,boolean:Ii,date:ht,datetime:ht,in:ft,integer:mt,like:pt,match:pt,ni:ft,null:zi,numeric:mt,removeFilter:qi,resetFilter:Ni,generateMinMax:bt},gt=f,ut;function Oi(e){clearTimeout(ut),ut=setTimeout(()=>{e.reload(),e.style?.theme&&mapp.ui.layers.legends[e.style.theme.type]?.(e),e.filter.list?.forEach(t=>{t.histogram?.update?.()}),e.mapview?.target.dispatchEvent(new Event("changeEnd"))},500)}function se(e,t){e.filter.current[t.field]&&(Object.hasOwn(e.filter.current[t.field],t.type)&&delete e.filter.current[t.field][t.type],Object.hasOwn(e.filter.current,t.field)&&delete e.filter.current[t.field],Number.isNaN(Number(t.Min))&&delete t.min,Number.isNaN(Number(t.Max))&&delete t.max),mapp.ui.layers.filters.applyFilter(e)}function qi(e,t){se(e,t),t.li?.classList.remove("selected"),t.card?.remove(),t.histogram?.removeChangeEnd?.(),delete t.selected,delete t.card,e.filter.list?.some(i=>i.card)||(e.filter.clearAll instanceof HTMLElement&&e.filter.clearAll.style.setProperty("display","none"),e.filter.resetAll instanceof HTMLElement&&e.filter.resetAll.style.setProperty("display","none"),e.filter.feature_count instanceof HTMLElement&&e.filter.feature_count.style.setProperty("display","none"))}async function Ni(e,t){if(se(e,t),!t.card)return;t.content=[await mapp.ui.layers.filters[t.type](e,t)].flat();let i=mapp.ui.elements.card(t);t.card.replaceWith(i),t.card=i}function pt(e,t){return mapp.utils.html.node`
  <input
    type="text"
    onkeyup=${i=>{i.target.value.length?(e.filter.current[t.field]??={},e.filter.current[t.field][t.type]=encodeURIComponent(`${t.leading_wildcard&&"%"||""}${i.target.value}`)):delete e.filter.current[t.field],f.applyFilter(e)}}>`}function Ii(e,t){function i(o){e.filter.current[t.field]={boolean:o},f.applyFilter(e)}return i(!1),mapp.ui.elements.chkbox({label:t.label||t.title||"chkbox",onchange:i})}function zi(e,t){function i(o){e.filter.current[t.field]={null:o},f.applyFilter(e)}return i(!1),mapp.ui.elements.chkbox({label:t.label||t.title||"chkbox",onchange:i})}async function bt(e,t){let i=mapp.utils.queryParams({layer:e,queryparams:{table:!0,template:"field_minmax",field:t.field},viewport:e.filter.viewport});i.filter=structuredClone(e.filter.current),delete i.filter[t.field];let o=mapp.utils.paramString(i),l=await mapp.utils.xhr(`${e.mapview.host}/api/query?${o}`),n=Number.isNaN(Number(t.min))?l.minmax[0]:t.min,a=Number.isNaN(Number(t.max))?l.minmax[1]:t.max;e.filter?.viewport&&(n=l.minmax[0],a=l.minmax[1]),t.min=t.type==="integer"?Math.round(n):Number.parseFloat(n),t.max=t.type==="integer"?Math.round(a):Number.parseFloat(a),t.min=t.val_a<t.min?t.val_a:t.min,t.max=t.val_b>t.max?t.val_b:t.max}async function mt(e,t){let i=e.infoj.find(n=>n.field===t.field);if(Object.assign(t,i),delete t.val_a,delete t.val_b,t.Min??=Number.isFinite(t.min)?t.min:void 0,t.Max??=Number.isFinite(t.max)?t.max:void 0,(Number.isNaN(Number(t.max))||Number.isNaN(Number(t.min)))&&await bt(e,t),Number.isNaN(Number(t.min))||Number.isNaN(Number(t.max)))return mapp.utils.html.node`<div>${mapp.dictionary.no_data_filter}</div>`;t.step??=t.type==="integer"?1:.01,e.filter.current[t.field]=Object.assign({gte:Number(t.min),lte:Number(t.max)},e.filter.current[t.field]),f.applyFilter(e);let o=t.prefix||t.suffix?`(${(t.prefix||t.suffix).trim()})`:"";return t.label_a??=`${mapp.dictionary.layer_filter_greater_than} ${o}`,t.label_b??=`${mapp.dictionary.layer_filter_less_than} ${o}`,t.val_a=e.filter.current?.[t.field]?.gte,t.val_b=e.filter.current?.[t.field]?.lte,t.callback_a=n=>{e.filter.current[t.field].gte=n,t.val_a=Number(n),f.applyFilter(e)},t.callback_b=n=>{e.filter.current[t.field].lte=n,t.val_b=Number(n),f.applyFilter(e)},t.slider=mapp.ui.elements.slider_ab(t),await Di(t,e),mapp.utils.html`
    ${t.histogram?.container}
    ${t.slider}`}async function Di(e,t){if(e.histogram&&(e.histogram===!0&&(e.histogram={}),e.histogram.container??=mapp.utils.html.node`<div>`,e.histogram.dataview??="histogram",e.histogram.options??={tooltip:!0},e.histogram.target??=e.histogram.container,e.histogram.layer??=t,e.histogram.queryparams??={field:e.field,table:!0,filter:!0},e.histogram.viewport??=t.filter.viewport,e.histogram.update||await mapp.ui.Dataview(e.histogram),e.histogram.viewport)){let i=function(){e.histogram.update()};t.mapview.Map.getTargetElement().addEventListener("changeEnd",i),e.histogram.removeChangeEnd=()=>{t.mapview.Map.getTargetElement().removeEventListener("changeEnd",i)}}}async function ft(e,t){if(se(e,t),!Array.isArray(t[t.type])){let l=structuredClone(e.filter?.current);delete l?.[t.field];let n=await mapp.utils.xhr(`${e.mapview.host}/api/query?`+mapp.utils.paramString({field:t.field,filter:l,layer:e.key,locale:e.mapview.locale.key,table:e.tableCurrent(),template:"distinct_values"}));if(!n)return console.warn(`Distinct values query did not return any values for field ${t.field}`),mapp.utils.html.node`<div>${mapp.dictionary.no_data_filter}</div>`;t[t.type]=[n].flat().map(a=>a[t.field]).filter(a=>a!==null)}let i=new Set(e.filter?.current[t.field]?.[t.type]||[]);if(t.dropdown||t.dropdown_pills||t.dropdown_search){let l=mapp.ui.elements.dropdown({callback:async(n,a)=>{a.length?Object.assign(e.filter.current,{[t.field]:{[t.type]:a}}):delete e.filter.current[t.field],f.applyFilter(e)},field:t.field,search:t.dropdown_search,entries:t[t.type].map(n=>({option:n,selected:i.has(n),title:n})),inputfilter:!0,keepPlaceholder:t.dropdown_pills,maxHeight:300,multi:!0,pills:t.dropdown_pills,placeholder:t.placeholder||mapp.dictionary.layer_filter_dropdown_select});return mapp.utils.html.node`<div class="filter">${l}`}if(t.searchbox){let l=mapp.utils.html.node`<div class="filter">`,n=mapp.ui.elements.pills({pills:e.filter?.current[t.field]?.[t.type],addCallback:(s,d)=>{Object.assign(e.filter.current,{[t.field]:{[t.type]:[...d]}}),f.applyFilter(e)},removeCallback:(s,d)=>{d.size===0?(a.input.value=null,delete e.filter.current[t.field]):Object.assign(e.filter.current,{[t.field]:{[t.type]:[...d]}}),f.applyFilter(e)},target:l}),a=mapp.ui.elements.searchbox({placeholder:mapp.dictionary.filter_searchbox_placeholder,searchFunction:s=>{if(a.list.innerHTML="",!s.target.value)return;let d=s.target.value,c=t[t.type].filter(u=>u.toString().toLowerCase().startsWith(d.toLowerCase()));if(!c.length){a.list.append(mapp.utils.html.node`
    ...
    <div class="legend">
      ${o}
      <div class=${t.legend.classList}>
        ${i}`,e.style.legend??=t.legend.node,e.style.legend&&e.style.legend.replaceChildren(...t.legend.node.children),t.legend.node}var vt={basic:de,categorized:re,distributed:y,graduated:ue};function pe(e){if(!e.target)return;let t={add:Bi,createGroup:Mi,groups:{},node:e.target};return typeof e.layers=="object"&&Object.values(e.layers).length&&Object.values(e.layers).forEach(i=>t.add(i)),t}function Bi(e){if(!e.hidden){if(mapp.ui.layers.view(e),!e.group){this.node.append(e.view),this.node.dispatchEvent(new CustomEvent("addLayerView",{detail:e}));return}this.groups[e.group]||this.createGroup(e),this.groups[e.group].addLayer(e),this.node.dispatchEvent(new CustomEvent("addLayerView",{detail:e}))}}function Mi(e){let t={list:[]};this.groups[e.group]=t,t.hideLayers=mapp.utils.html.node`<button
  class="notranslate material-symbols-outlined active"
    title=${mapp.dictionary.layer_group_hide_layers}
    onclick=${i=>{i.target.style.visibility="hidden",t.list.filter(o=>o.display).forEach(o=>o.hide())}}>visibility_off`,t.meta=mapp.utils.html.node`<div class="meta no-select">`,t.drawer=mapp.ui.elements.drawer({class:`layer-group ${e.groupClassList||""}`,content:t.meta,data_id:e.group,header:mapp.utils.html`
       <h2>${e.group}</h2>
       ${t.hideLayers}`}),this.node.appendChild(t.drawer),t.chkVisibleLayer=Pi,t.addLayer=Vi}function Vi(e){if(e.groupmeta){let t=mapp.utils.html.node`<div>`;t.innerHTML=e.groupmeta,mapp.utils.render(this.meta,t)}this.list.push(e),this.drawer.appendChild(e.view),this.chkVisibleLayer(),e.showCallbacks.push(()=>this.chkVisibleLayer()),e.hideCallbacks.push(()=>this.chkVisibleLayer())}function Pi(){this.hideLayers.style.visibility=this.list.some(e=>e.display)?"visible":"hidden"}function me(e){let t=[];for(let[o,l]of Object.entries(e.dataviews))if(!(typeof l=="string"||l===!0||l===!1)){if(Object.assign(l,{host:e.mapview.host,key:o,layer:e}),Hi(l),l.tabview&&mapp.utils.mobile()||(l.label??=l.title||l.key,mapp.ui.utils.dataviewDialog(l),l.target=l.target instanceof HTMLElement?l.target:mapp.utils.html.node`
    ...
    <p style="display:none">${e.filter.count} ${e.filter.count_meta}`,e.filter.viewport_description??=e.filter.viewport?mapp.dictionary.filter_in_viewport:mapp.dictionary.filter_not_in_viewport,e.filter.viewport_description=mapp.utils.html.node`<p style="display:none"><i>${e.filter.viewport_description}</i>`,e.filter.content=[e.filter.dropdown,e.filter.clearAll,e.filter.resetAll,e.filter.feature_count,e.filter.viewport_description],e.filter.dialog&&(e.filter.filter_list=mapp.utils.html.node`
      <div id=filter-list class=filter-list>`,e.filter.content=[mapp.utils.html`<p class=bold>${e.name}</p>`,e.filter.feature_count,e.filter.viewport_description,e.filter.clearAll,e.filter.resetAll,e.filter.dropdown,e.filter.filter_list],e.filter.content=mapp.utils.html.node`${e.filter.content}`,e.filter.dialog&&(e.filter.dialog={btn_label:"Filtering",title:"Filtering",icon:"filter_alt",view:e.filter.filter_list,...e.filter.dialog}),e.filter.clearAll.style.display="inline-block",e.filter.resetAll.style.display="inline-block"),e.filter.drawer={drawer:e.filter.drawer,data_id:"filter-drawer",class:e.filter.classList||"",header:mapp.utils.html`
        <h3>${mapp.dictionary.filter_header}</h3>`,content:e.filter.dialog?.btn||e.filter.content,popout:e.filter.popout,dialog:e.filter.dialog,view:e.view,layer:e},e.filter.view=mapp.ui.elements.drawer(e.filter.drawer),e.filter.view}var wt={numeric:"numeric",integer:"integer",text:"like",date:"date",datetime:"datetime",boolean:"boolean"};function Ri(e){let t=[];e.filter.include??=[],e.filter.exclude??=[];for(let i of e.infoj)if(!(i.skipEntry===!0||i.field===void 0||e.filter.exclude.includes(i.field))){if(e.filter.includeAll||e.filter.include.includes(i.field)||i.filter===!0){if(i.type??="text",!Object.hasOwn(wt,i.type))continue;Object.keys(i.filter||{}).length||(i.filter=wt[i.type])}typeof i.filter=="string"&&(i.filter={type:i.filter}),!(!i.filter?.type||!Object.hasOwn(mapp.ui.layers.filters,i.filter.type))&&(i.filter.title??=i.title,i.filter.field??=i.field,t.push(structuredClone(i.filter)))}return t}function L(e){e.display&&(clearTimeout(e.filter.debounce),e.filter.debounce=setTimeout(async()=>{for(let t of e.filter.list){if(!Object.hasOwn(e.filter.current,t.field)||t.min===void 0||t.max===void 0||t.Min&&t.Max||(await mapp.ui.layers.filters.generateMinMax(e,t),t.max-t.min===0))continue;let o=mapp.ui.elements.slider_ab(t);t.slider.replaceWith(o),t.slider=o}mapp.ui.utils.locationCount(e).then(t=>{let i=mapp.utils.formatNumericValue({value:t});e.filter.count.innerText=i,e.filter.feature_count.style.setProperty("display","block")})},1e3))}function Ui(e){e.multi_filter&&(mapp.layer.multi_filter||(delete e.filter.hidden,e.filter.dialog=!0,e.filter.drawer=!1,e.filter.includeAll=!0))}var $t=e=>{let t={layer:e.key,mapview:e.mapview,target:mapp.utils.html.node`<div>`,...e.gazetteer};return mapp.ui.Gazetteer(t),t.target};function ge(e){e.jsonEditor===!0&&(e.jsonEditor={});let t=mapp.utils.html.node`<button class="wide flat" data-id="jsonEditor">JSON Editor</button>`;return t.addEventListener("click",async()=>{t.classList.toggle("active");let i=mapp.utils.jsonParser(e),o=mapp.utils.html.node`<div>`,l={className:"text-button",onClick:()=>Wi(n,e),text:"Update Layer",title:"Update Layer",type:"button"},n=await mapp.ui.utils.layerJSE(o,{json:i},l);e.jsonEditor.dialog=mapp.ui.elements.dialog({header:mapp.utils.html.node`<div>`,css_style:"width: 500px; height 300px;",containedCentre:!0,contained:!0,closeBtn:!0,onClose:()=>{t.classList.toggle("active")},content:o})}),e.removeCallbacks.push(i=>{i.jsonEditor?.dialog?.close()}),t}async function Wi(e,t){let i=e.get(),o=JSON.parse(i.text);await t.update(o)}function be(e){let t=mapp.utils.html.node`<p data-id="meta" class="meta no-select">`;return t.innerHTML=e.meta,t}function ve(e){let t=[];for(let[o,l]of Object.entries(e.reports)){if(typeof l=="boolean")continue;l.key=o,l.label??=o,l.layer=e,l.template&&(l.report={template:l.template});let n=mapp.ui.elements.reportLink(l);t.push(n)}return e.reports.dialog&&(e.reports.dialog={btn_label:"Reports",title:"Reports",icon:"summarize",...e.reports.dialog}),mapp.ui.elements.drawer({drawer:e.reports.drawer,class:e.reports.classList||"",data_id:"reports-drawer",header:mapp.utils.html`
    ...
    <div class="link-with-img">
      ...
      <input
        type="file"
        accept="image/*;capture=camera" onchange=${t=>{w(t,e)}}>`}function to(e){let t=[];if(e.value?.map(i=>{let o=e.edit&&mapp.utils.html`<button title="${mapp.dictionary.delete}"
      class="notranslate material-symbols-outlined color-danger delete"
      ...
      <input
        type="file"
        accept="image/*;capture=camera" onchange=${o=>{w(o,e)}}>`;t.push(i)}if(t.length)return mapp.utils.html.node`<div class="images-grid">${t}`}function io(e){let t=[];if(e.value?.map(i=>{let o=e.edit&&mapp.utils.html`<button 
      title="${mapp.dictionary.delete}"
      class="notranslate material-symbols-outlined color-danger delete"
      data-name=${i.replace(/^.*\//,"").replace(/\.([\w-]{3})/,"")}
      data-href=${i}
    ...
    <div class="${t}">`,e.target=e.locationViewTarget}var Ce=ao;function ao(e){if(e.edit){e.newValue===!0&&(e.newValue=mapp.utils.temporal.dateToUnixEpoch(),e.location.view?.dispatchEvent(new CustomEvent("valChange",{detail:e})));let o=e.newValue||e.value;typeof o=="string"&&console.warn(`${e.type} field: ${e.field} should be an integer.`);let l=mapp.utils.temporal[e.type](Number.parseInt(o));return mapp.utils.html.node`<input
    ...
    <button 
      class=${t}
      title="${mapp.dictionary.layer_visibility}"
      onclick="${o=>{o.target.classList.toggle("active")?e.location.layer.show():e.location.layer.hide()}}">${e.location.layer.name}`}function E(e){return ho(e),e.mapview??=e.location.layer.mapview,go(e),e.zIndex??=e.location.layer.zIndex++,bo(e),e.panel??=mapp.utils.html.node`<div class="entry-layer">`,e.panel}function ho(e){e.value!==void 0&&(e.data=e.value,e.data?.length===0&&(e.data=null)),(e.featureSet||e.featureLookup)&&(!e.query&&!e.data?(e.disabled=!0,e.display_toggle?.classList.add("disabled")):(e.disabled=!1,e.display_toggle?.classList.remove("disabled")))}function go(e){if(!e.layer)return;e.key=e.layer;let t=structuredClone(e.mapview.locale.layers.find(o=>o.key===e.layer));if(!t){console.warn(`Layer [${e.layer}] not found in mapview.locale`);return}let i=Object.keys(t).filter(o=>!Object.hasOwn(e,o)).reduce((o,l)=>(o[l]=t[l],o),{});Object.assign(e,i),e.params??={},e.params.layer_template??=e.layer,delete e.layer}async function bo(e){if(e.L&&e.display){e.show();return}else if(e.L)return;e.source_key=e.key,await mapp.layer.decorate(e),e.L&&(e.location.Layers.push(e),e.key+=`-${e.location.id}`,e.L.set("key",e.key),e.show=vo,e.hide=$o,e.mapview.layers[e.key]=e,e.display_toggle=mapp.ui.elements.chkbox({checked:e.display,data_id:`${e.key}-display`,disabled:e.disabled,label:e.name,onchange:t=>{t?e.show():e.hide()}}),e.tables&&(e.tableCurrent()||e.display_toggle.classList.add("disabled"),e.changeEndCallbacks.push(()=>{e.tableCurrent()?e.display_toggle.classList.remove("disabled"):e.display_toggle.classList.add("disabled")})),e.panel.append(e.display_toggle),e.meta&&e.panel.append(mapp.ui.layers.panels.meta(e)),e.style.elements??=["labels","label","hovers","hover","icon_scaling","opacitySlider","themes","theme"],e.style.panel=mapp.ui.elements.layerStyle.panel(e),e.style.default&&(e.style.default={...e.location?.style,...e.style.default}),e.style.panel?.style.setProperty("display","none"),e.style.panel&&e.panel.append(e.style.panel),e.location.removeCallbacks.push(()=>{typeof e.remove=="function"&&e.remove()}),e.display&&e.show())}async function vo(){let e=this;if(e.query){let t=mapp.utils.queryParams(e),i=mapp.utils.paramString(t);if(e.data=await mapp.utils.xhr(`${e.mapview.host}/api/query?${i}`),!e.data){e.disabled=!0,e.display_toggle?.classList.add("disabled"),e.panel.replaceChildren(e.display_toggle);return}}wo(e),e.style.panel?.style.setProperty("display","block"),e.display=!0;try{e.mapview.Map.addLayer(e.L)}catch{}typeof e.reload=="function"&&e.reload(),e.showCallbacks?.forEach(t=>typeof t=="function"&&t(e))}function wo(e){if(e.data){if(e.featureSet){let t=Array.isArray(e.data)?e.data:[e.data];e.featureSet=new Set(t);return}if(e.featureLookup){e.featureLookup=Array.isArray(e.data)?e.data:[e.data];return}e.features=e.data,e.setSource(e.features)}}function $o(){this.display=!1,this.style.panel?.style.setProperty("display","none"),this.mapview.Map.removeLayer(this.L),this.hideCallbacks?.forEach(e=>typeof e=="function"&&e(this))}function S(e){let t=mapp.ui.elements.reportLink(e);return mapp.utils.html.node`<div class="val">${t}`}function j(e){if(e.edit)return _o(e);if(!(e.value===null||Number.isNaN(Number(e.value))))return mapp.utils.formatNumericValue(e),mapp.utils.html.node`<div 
    class="val"
    style=${e.css_val}
    >${e.stringValue}`}function _o(e){e.edit.callback??=()=>{e.location.view?.dispatchEvent(new CustomEvent("valChange",{detail:e}))},e.edit.min??=e.min||-2147483648,e.edit.max??=e.max||2147483647,e.edit.step??=e.step||e.type==="integer"?1:.1,Object.assign(e,e.edit);let t=e.value??e.newValue;return e.edit.range&&!Number.isNaN(Number(t))&&t>e.min&&t<e.max?mapp.ui.elements.slider(e):mapp.ui.elements.numericInput(e)}var Et=e=>(e.pills??=e.value||[],mapp.ui.elements.pills(e),e.container);function Te(e){if(!Array.isArray(e.value)){console.warn("Entry type pin requires a value array.");return}return e.srid??=e.location.layer.srid,e.display??=e.display===void 0,e.zIndex??=1/0,e.Style??=e.style?mapp.utils.style(e.style):e.location.pinStyle,e.show??=xo,e.hide??=ko,e.getExtent??=yo,e.geometry={coordinates:e.value,type:"Point"},e.display&&e.show(),e.label??=mapp.dictionary.pin,e.chkbox??=mapp.ui.elements.chkbox({checked:e.display,label:e.label,onchange:t=>{t?e.show():e.hide()}}),e.chkbox}function xo(){this.disabled||(this.location.removeLayer(this),this.display=!0,this.L=this.location.layer.mapview.geoJSON(this),this.location.Layers.push(this))}function ko(){this.display=!1,this.location.removeLayer(this)}function yo(){return this.buffer??=0,this.buffer=Number.parseInt(this.buffer)||200,ol.extent.buffer(this.L.getSource().getExtent(),this.buffer)}function Oe(e){return e.textarea||(e.textarea=mapp.utils.html.node`<textarea
      class="val"
  ...
  <div
    ...
    style="${e.css_val}">${t}`;return!e.edit&&e.type==="html"&&(i.innerHTML=e.value||""),i}function Ie(e){let t,i=e.value?.toString().replace(".",":");return i=i&&i.length<3&&`${i}:00`||i,e.edit?t=mapp.utils.html.node`
      <input
        type="time"
        value=${i}
        onchange=${l=>{e.newValue=Number.parseFloat(l.target.value.replace(":",".")),e.location.view?.dispatchEvent(new CustomEvent("valChange",{detail:e}))}}>`:t=i,e.css_val??="",mapp.utils.html.node`
    <div
      class="val"
      style="${e.css_val}">
      ${t}`}function ze(e){return mapp.utils.html.node`
        <div
          class="label"
          style=${e.css_title}
          title=${e.tooltip}>${e.title}`}function To(){console.warn("The type:defaults entry method has been deprecated.")}function jt(e){return console.warn(`The type:${e.type} entry method has been deprecated in favour of the type:layer entry method.`),e.type="layer",e.qID??="id",e.name??=e.label,E(e)}var Tt={boolean:_e,dataview:Le,date:Ce,datetime:Ce,defaults:To,documents:C,geometry:Ee,html:T,image:C,images:C,integer:j,json:Se,key:je,layer:E,link:S,mvt_clone:jt,numeric:j,pills:Et,pin:Te,ping:Oe,query_button:qe,report:S,tab:St,text:Ne,textarea:T,time:Ie,title:ze,vector_layer:jt};var O;function De(e,t){if(!e.infoj)return;let i=mapp.utils.html.node`<div class="location-view-grid">`;O={},t??=e?.layer?.infoj_order;let o=Array.isArray(t)?t.map(l).filter(a=>a!==void 0):e.infoj;function l(a){if(typeof a=="string"){let s=e.infoj.find(d=>new Set([d.key,d.field,d.query]).has(a));return s||console.warn(`infoj_order field: "${a}" not found in location.infoj. Please add entry.key, entry.field, or entry.query to the entry.`),s}else if(typeof a=="object")return a.location=e,a}let n=0;for(let a of o){if(a.key??=a.field||n++,e.view?.classList.contains("disabled"))break;if(a.listview=i,a.type??="text",a.edit===!0&&(a.edit={}),Oo(a),qo(a),Ot(a)||(No(a),Io(a),zo(a),Do(a),Fo(a),Ao(a)))continue;if(!Object.hasOwn(mapp.ui.locations.entries,a.type)){console.error(`entry.type:${a.type} method not found.`);continue}let s=mapp.ui.locations.entries[a.type]?.(a);s&&a.node.append(s),Mo(a)}return i}function Oo(e){e.jsonb_field&&e.jsonb_key&&e.value!==null&&typeof e.value=="object"&&e.value.jsonb&&(e.value=e.value.jsonb[e.jsonb_field][e.jsonb_key])}function qo(e){if(e.objectAssignFromField&&e.objectMergeFromField){console.warn(`${e.key}: objectAssignFromField and objectMergeFromField cannot both be used on the same entry.`);return}let t=e.objectAssignFromField||e.objectMergeFromField||e.json_field;if(!t)return;let i=e.location.infoj.find(o=>o.field===t);if(i&&typeof i.value=="object"){if(e.json_field){if(!e.json_key){console.warn("json_field requires entry.json_key to be specified");return}if(!i.value)return;e.value=i.value[e.json_key]}e.objectAssignFromField&&Object.assign(e,i.value),e.objectMergeFromField&&mapp.utils.merge(e,i.value)}}function Ot(e){if(e.skipEntry||e.skipFalsyValue&&!e.value&&!e.edit||e.skipUndefinedValue&&typeof e.value>"u"&&!e.edit||e.skipNullValue&&e.value===null&&!e.edit)return!0}function No(e){e.nullValue!==void 0&&(e.edit||(e.value??=e.nullValue))}function Io(e){e.default!==void 0&&e.edit&&(e.newValue=e.default,e.location.view?.dispatchEvent(new CustomEvent("valChange",{detail:e})))}function zo(e){e.group&&(O[e.group]||(O[e.group]=e.listview.appendChild(mapp.ui.elements.drawer({data_id:"location-infoj-group",class:`group ${e.groupClassList||""}`,header:mapp.utils.html`
          <h3>${e.group}</h3>`}))),e.listview=O[e.group])}function Do(e){let t=`contents ${e.type} ${e.class||""} ${e.inline&&"inline"||""}`;e.node=e.listview.appendChild(mapp.utils.html.node`
  <div
    data-type=${e.type}
    class=${t}>`)}function Fo(e){e.title&&e.node.append(mapp.ui.locations.entries.title(e))}function Ao(e){if(e.query)if(e.queryCheck??=e.run,e.queryCallback??=Bo,e.runOnce||e.queryCheck){delete e.runOnce,e.host??=e.location.layer?.mapview?.host||mapp.host;let t=mapp.utils.queryParams(e),i=mapp.utils.paramString(t);return e.url=`${e.host}/api/query?${i}`,e.blocking&&e.location.view?.classList.add("loading"),mapp.utils.xhr(e).then(o=>e.queryCallback(o,e)),!0}else e.field&&!e.xhr&&console.warn(`field:"${e.field}" has a query:"${e.query}" which is not set to run, please add the run or runOnce flag to the entry.`)}function Bo(e,t){if(t.blocking&&t.location.view?.classList.remove("loading"),e?t.value=t.field?e[t.field]:e:t.value=t.nullValue||null,Ot(t)){t.node.remove();return}let i=mapp.ui.locations.entries[t.type]?.(t);i&&t.node.appendChild(i)}function Mo(e){if(!e.tooltip)return;let t=e.node.querySelector("label")||e.node.querySelector("div.label");t&&(e.tooltipElement||(e.tooltipElement=mapp.ui.elements.tooltip({content:e.tooltip}),t.append(e.tooltipElement)))}var Vo=[{symbol:"A",colour:"#2E6F9E"},{symbol:"B",colour:"#EC602D"},{symbol:"C",colour:"#5B8C5A"},{symbol:"D",colour:"#B84444"},{symbol:"E",colour:"#514E7E"},{symbol:"F",colour:"#E7C547"},{symbol:"G",colour:"#368F8B"},{symbol:"H",colour:"#841C47"},{symbol:"I",colour:"#61A2D1"},{symbol:"J",colour:"#37327F"},{symbol:"K",colour:"#FFA500"},{symbol:"L",colour:"#7FFF00"},{symbol:"M",colour:"#FFD700"},{symbol:"N",colour:"#00CED1"},{symbol:"O",colour:"#9932CC"},{symbol:"P",colour:"#FF69B4"},{symbol:"Q",colour:"#008B8B"},{symbol:"R",colour:"#8A2BE2"},{symbol:"S",colour:"#FF4500"},{symbol:"T",colour:"#00FF00"},{symbol:"U",colour:"#FF00FF"},{symbol:"V",colour:"#708090"},{symbol:"W",colour:"#F4A460"},{symbol:"X",colour:"#000080"},{symbol:"Y",colour:"#D2691E"},{symbol:"Z",colour:"#8FBC8F"}];function Fe(e){e.mapview||console.warn("A mapview is required in the locations listview params argument."),e.target||console.warn("A target element is required in the locations listview params argument.");let t=e.mapview.locale;t.locations??={},t.locations.pinStyle??={anchor:[.5,1],scale:3,type:"markerLetter"},t.locations.style??={fillColor:"#fff",fillOpacity:.1,strokeColor:"#fff"},t.locations.records??=t.listview_records||structuredClone(Vo),t.locations.clearAll=e.target.appendChild(mapp.utils.html.node`
    <button
    ...
    `);async function l(){e.infoj.some(a=>typeof a.newValue<"u")&&!await mapp.ui.elements.confirm({text:mapp.dictionary.location_close_without_save})||e.remove()}e.view=mapp.ui.elements.drawer({data_id:`location-${e.layer.key}-${e.id}`,class:"location-view raised expanded",header:o}),e.viewEntries=e.view.appendChild(mapp.ui.locations.infoj(e)),e.view.querySelector(".header").style.borderBottom=`3px solid ${e.record.colour}`,e.view.addEventListener("valChange",Ro),e.renderLocationView=Uo,e.view.addEventListener("render",()=>e.renderLocationView()),e.view.addEventListener("updateInfo",()=>{e.editToggle&&(e.editToggle.classList.remove("toggle-on"),e.removeEdits()),e.layer?.dataviews&&Object.values(e.layer.dataviews).forEach(n=>{n.display===!0&&n.update()}),e.renderLocationView()})}function Ho(e){if(e.layer?.toggleLocationViewEdits===!1)return e.removeEdits(),!1;if(!e.layer?.toggleLocationViewEdits||!e.infoj.some(o=>o.edit))return!1;!e.new&&e.removeEdits();let t=`notranslate material-symbols-outlined ${e.new?"toggle-on":""}`;return e.editToggle=mapp.utils.html.node`<button
    title="Enable edits"
    class=${t}
    onclick=${i}>edit`,!0;async function i(o){await e.confirmUpdate()===void 0&&(o.target.classList.toggle("toggle-on")?e.restoreEdits():e.removeEdits(),e.renderLocationView())}}function Ro(e){let t=e.detail,i=t.location;if(typeof t.valChangeMethod=="function"){t.valChangeMethod(t);return}if(t.value!=t.newValue?t.node?.classList.add("val-changed"):(delete t.newValue,t.node?.classList.remove("val-changed")),i.infoj.some(o=>o.invalid)){i.view.querySelector(".btn-save").style.display="none";return}i.view.querySelector(".btn-save").style.display=i.infoj.some(o=>typeof o.newValue<"u")?"inline-block":"none"}function Uo(){let e=this;e.view.querySelector(".btn-save").style.display="none",e.viewEntries.remove(),e.view.classList.remove("disabled"),e.viewEntries=e.view.appendChild(mapp.ui.locations.infoj(e))}var qt={entries:Tt,infoj:De,listview:Fe,view:Ae};function Be(e){if(e.node)return e.tabs=e.node.appendChild(mapp.utils.html.node`<div class="tabs">`),e.panel=e.node.appendChild(mapp.utils.html.node`<div class="panel">`),e.id&&(e.node.dataset.id=e.id),e.addTab=Wo,e.node.addEventListener("addTab",t=>e.addTab(t.detail)),e}function Wo(e){if(e.tab&&!e.dynamic)return;let t=this;e.layer?.dataviews?.[e.key]&&(e.disableTabClose??=e.layer.dataviews.hide),e.activate??=n,e.location?e.location.removeCallbacks.push(()=>e.remove()):e.layer&&(e.layer.showCallbacks.push(()=>{e.display&&e.show()}),e.layer.hideCallbacks.push(()=>{e.remove()})),e.label??=e.title||e.key||"Tab",e.tab_btn=mapp.utils.html.node`<button
    ...
    overflow: hidden !important`;Object.assign(e.dataview_dialog,{header:mapp.utils.html.node`<h1> ${e.label}`,data_id:`${e.key}-dataviews-dialog`,target:document.getElementById("Map"),height:"auto",left:"5%",top:"0.5em",class:"box-shadow tabview",css_style:t,containedCentre:!0,contained:!0,headerDrag:!0,closeBtn:!0,onClose:()=>{e.chkbox&&(e.chkbox.querySelector("input").checked=!1)}}),mapp.ui.elements.dialog(e.dataview_dialog);let i=e.dataview_dialog.node.querySelector(".content");i.classList.add("panel"),e.target=i,e.dataview_dialog.dialog.close(),e.show=()=>{e.update(),e.toolbar&&e.dataview_dialog.node.querySelector(".btn-row").style.removeProperty("display"),e.chart&&(e.dataview_dialog.node.appendChild(e.target.querySelector("canvas")),e.dataview_dialog.node.removeChild(e.target)),e.dataview_dialog.dialog.show()},e.hide=()=>{e.dataview_dialog.dialog.close()},e.location?.removeCallbacks?.push?.(()=>e.dataview_dialog.dialog.close())}var tl={create:il},It=tl;async function il(e){e.query??="histogram",e.options??={};let t=`column-gap: ${e.options.columnGap||10}px`;e.node=mapp.utils.html.node`<div class="histogram" style=${t}>`,e.options.width&&(e.node.style.width=`${e.options.width}px`),e.options.height&&(e.node.style.height=`${e.options.height}px`),e.options.title&&e.target.append(mapp.utils.html.node`<h3 class="histogram-title">${e.options.title}`),e.setData=ll,e.target.append(e.node),e.options.xlabel&&(e.xlabel=mapp.utils.html.node`<div class="histogram-xlabel">${e.options.xlabel}`,e.options.width&&(e.xlabel.style.width=`${e.options.width}px`),e.target.append(e.xlabel)),e.options.caption&&(e.caption=mapp.utils.html.node`<span class="histogram-caption">${e.options.caption}`,e.target.append(e.caption))}function ll(e){this.data=e,nl(this)}function nl(e){if(!Array.isArray(e.data)){mapp.utils.render(e.node,mapp.utils.html`<div>`);return}let t=Math.max(...e.data.map(l=>l.count));e.options.height??=100;let i=e.data.map(l=>{let n=`height: ${Math.round(e.options.height*l.count/t)}px`,a="";return e.options.tooltip&&(a=mapp.utils.html`<div class="bucket-info">
      ...
      style=${n}>
      ${a}`}),o=e.options.ylabel?mapp.utils.html`<div class="ylabel">${e.options.ylabel}`:"";mapp.utils.render(e.node,mapp.utils.html`${o}${i}`)}var m={idle:600};function He(e){Object.assign(m,e),m.idle!==0&&(window.onload=g,window.onmousemove=g,window.onmousedown=g,window.ontouchstart=g,window.onclick=g,window.onkeypress=g,g(),Dt())}function g(){m.locked||(m.timeout&&clearTimeout(m.timeout),m.timeout=setTimeout(zt,m.idle*1e3))}function zt(){m.locked=!0,m.renew&&clearTimeout(m.renew);let e={url:`${m.host}/api/user/cookie?destroy=true`};e.onLoad=t=>location.reload(),mapp.utils.xhr(e)}function Dt(){m.renew=setTimeout(al,(m.idle-20)*1e3)}function al(){let e={url:`${m.host}/api/user/cookie?renew=true`};e.onLoad=t=>{if(t.target.status===401)return zt();Dt()},mapp.utils.xhr(e)}function Re(e){let t=mapp.utils.html.node`
  <div class="interface-mask">
    ...
    <button 
      ...
      onclick=${i=>i.target.closest(".interface-mask").remove()}>close</button>`;document.body.append(t)}var Ft={create:sl,toolbar:{copyToClipboard:rl,csvupload:ul,jsonfile:cl,sqlinsert:pl}};async function sl(e){e.props??={mainMenuBar:!1,mode:"text",readOnly:!0},e.jsoneditor=await mapp.ui.elements.jsoneditor(e),e.setData=dl,e.data&&e.setData(e.data)}function dl(e){this.data=e;let t={json:e};this.jsoneditor.set(t)}function cl(e){return e.title??=e.label||"Unknown",mapp.utils.html.node`<button onclick=${()=>{let i={filename:`${e.title}.json`,text:JSON.stringify(e.data,null,2),type:"application/json"};mapp.utils.textFile(i)}}>Download</button>`}function rl(e){return mapp.utils.html.node`<button onclick=${()=>{let i=JSON.stringify(e.data,null,2);mapp.utils.copyToClipboard(i)}}>Copy to Clipboard</button>`}function ul(e){return e.toolbar.csvupload.label??="CSV Upload",e.toolbar.csvupload.input=mapp.utils.html.node`<input 
...
`)}`});return}e.setData(i)}}>`,e.toolbar.csvupload.input}function pl(e){return e.props={mainMenuBar:!1,mode:"text"},mapp.utils.html.node`<button onclick=${()=>{let i=e.jsoneditor.get();if(i.text==="")return;let o=mapp.utils.paramString(e.toolbar.sqlinsert),l=`${mapp.host}/api/query?${o}`;return mapp.utils.xhr({body:i.text,method:"POST",url:l})}}>SQL Insert</button>`}async function Ue(e,t,i){return await mapp.ui.elements.jsoneditor({props:{onRenderMenu:l=>ml(l,i),content:t,mode:"text"},target:e})}function ml(e,t){return e.push(t),e.filter(i=>i.text!=="text").filter(i=>i.text!=="tree").filter(i=>i.text!=="table").filter(i=>i.type!=="separator").filter(i=>i.className!=="jse-undo").filter(i=>i.className!=="jse-redo").filter(i=>i.className!=="jse-search").filter(i=>i.className!=="jse-contextmenu").filter(i=>i.className!=="jse-sort").filter(i=>i.className!=="jse-transform")}async function q(e){if(Array.isArray(e.Features)&&!e.filter?.viewport)return e.Features.length;let t={layer:e,queryparams:{}};t.viewport=e.filter?.viewport||e.queryparams?.viewport;let i=mapp.utils.queryParams(t),o=mapp.utils.paramString({...i,filter:e.filter?.current,layer:e.key,table:e.tableCurrent(),qID:e.qID,template:"location_count"}),l=await mapp.utils.xhr(`${e.mapview.host}/api/query?${o}`);if(l instanceof Error)if(e.filter?.viewport){console.warn("locationCount failed on viewport.");return}else return e.filter.viewport=!0,await q(e);return l}var We={},r={X:{min:75,lockSize:200,grid:"gridTemplateColumns"},Y:{min:0,lockSize:50,grid:"gridTemplateRows"},gridTemplate:{X:"auto",Z:"10px",Y:"auto"}};function Je(e){e.resizeEvent??=fl.bind(e),e.stopResize=gl.bind(e),e.axis??="X",e.doubleClickClose&&(e.target.ondblclick=t=>hl(t,e)),e.target.addEventListener("mousedown",t=>{t.preventDefault(),document.body.style.cursor="grabbing",globalThis.addEventListener("mousemove",e.resizeEvent),globalThis.addEventListener("mouseup",e.stopResize)}),e.target.addEventListener("touchstart",t=>{t.preventDefault(),globalThis.addEventListener("touchmove",e.resizeEvent),globalThis.addEventListener("touchend",e.stopResize)},{passive:!0}),We.gridTemplateColumns=getComputedStyle(document.body).gridTemplateColumns,We.gridTemplateRows=getComputedStyle(document.body).gridTemplateRows}function fl(e){let t=this.axis;r.X.size=e.touches?.[0]?.pageX||e.pageX,r.Y.size=globalThis.innerHeight-(e.touches?.[0]?.pageY||e.pageY);let i=r[t];if(i.size<i.min){r.gridTemplate[t]="0px",document.body.style[i.grid]=Object.values(r.gridTemplate).join(" ");return}i.size<i.lockSize||(t==="X"&&r.X.size>globalThis.innerWidth/2&&(r.X.size=globalThis.innerWidth/2),t==="Y"&&(r.Y.size>globalThis.innerHeight-10&&(r.Y.size=globalThis.innerHeight),OL.style.marginTop=`-${r.Y.size/2}px`),r.gridTemplate[t]=`${i.size}px`,document.body.style[i.grid]=Object.values(r.gridTemplate).join(" "),r.gridTemplate={X:"auto",Z:"10px",Y:"auto"})}function hl(e,t){if(r.X.size=e.touches?.[0]?.pageX||e.pageX,r.Y.size=globalThis.innerHeight-(e.touches?.[0]?.pageY||e.pageY),r[t.axis].size<r[t.axis].min){document.body.style[r[t.axis].grid]=We[r[t.axis].grid];return}r.gridTemplate[t.axis]="0px",document.body.style[r[t.axis].grid]=Object.values(r.gridTemplate).join(" "),r.gridTemplate={X:"auto",Z:"10px",Y:"auto"}}function gl(){document.body.style.cursor="auto",globalThis.removeEventListener("mousemove",this.resizeEvent),globalThis.removeEventListener("touchmove",this.resizeEvent),globalThis.removeEventListener("mouseup",this.stopResize),globalThis.removeEventListener("touchend",this.stopResize)}var At={cssColour:Ve,dataview:Nt,dataviewDialog:Pe,histogram:It,idleLogout:He,imagePreview:Re,Json:Ft,layerJSE:Ue,locationCount:q,resizeHandler:Je};var Bt={Dataview:N,elements:ct,Gazetteer:rt,layers:kt,locations:qt,Tabview:Be,utils:At};globalThis.ui=Bt;globalThis.mapp??={};mapp.ui=Bt;
